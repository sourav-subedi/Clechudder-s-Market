...
try {
    ...
    // Get recent orders (last 5) - Fixed table name and added ROWNUM limit
    $ordersQuery = "SELECT * FROM (
                        SELECT o.*, u.full_name AS customer_name, u.email AS customer_email,
                               TO_CHAR(o.order_date, 'YYYY-MM-DD') as formatted_date
                        FROM orders o
    ...
    // Get order statistics - Fixed to handle Oracle's uppercase column names
    $statsQuery = "SELECT 
        COUNT(*) AS total_orders,
        NVL(SUM(total_amount), 0) AS total_sales,
        SUM(CASE WHEN UPPER(status) = 'COMPLETED' THEN 1 ELSE 0 END) AS completed_orders,
        SUM(CASE WHEN UPPER(status) = 'PENDING' THEN 1 ELSE 0 END) AS pending_orders
        FROM orders";
                ...
                <?php if (!empty($recentOrders)): ?>
                    <?php foreach ($recentOrders as $order): ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($order['FORMATTED_DATE'] ?? 'N/A'); ?></td>
                            <td>$<?php echo isset($order['TOTAL_AMOUNT']) ? number_format($order['TOTAL_AMOUNT'], 2) : '0.00'; ?></td>
                            <td class="status-<?php echo strtolower(htmlspecialchars($order['STATUS'] ?? 'unknown')); ?>">
                            ...
                            <td>
                                ...
                                <?php elseif (isset($order['STATUS']) && strtolower($order['STATUS']) == 'processing'): ?>
                                    <form class="action-form" method="POST">
                                        <input type="hidden" name="order_id" value="<?php echo $order['ORDER_ID']; ?>">
                                ...
                                <?php endif; ?>
                                <?php if (isset($order['STATUS']) && !in_array(strtolower($order['STATUS']), ['completed', 'cancelled'])): ?>
                                    <form class="action-form" method="POST">
                                        <input type="hidden" name="order_id" value="<?php echo $order['ORDER_ID']; ?>">
